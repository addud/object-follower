/**
 * Main program code for the ramboot utility.
 *
...
int main(int argc, char *argv[])
{
  ...
  printf("#=========================================================#\n");
  printf("#                    SRAM Boot Utility                    #\n");
  printf("#=========================================================#\n\n");

  if (argc != 2)
    {
      printf("Syntax: %s <Application program image to write into SRAM>\n"
             "\n"
             "Example: %s foo_samba_ram.bin\n", argv[0], argv[0]);
      exit(1);
    }
  ...
  printf("NXT device in reset mode located and opened.\n"
         "Starting program now from SRAM...\n");
  ...
  NXT_HANDLE_ERR(nxt_jump(nxt, 0x202000), nxt,
                 "Error jumping to application program");
