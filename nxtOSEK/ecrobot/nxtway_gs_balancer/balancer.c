...
static F32 ud_psi;                /* 車体ピッチ角度(ψ)状態値 */
...
// 関数名  : balance_control
// 引数    :
//   (F32)args_cmd_forward : 前進/後進命令。100(前進最大値)～-100(後進最大値)
//   (F32)args_cmd_turn    : 旋回命令。100(右旋回最大値)～-100(左旋回最大値)
//   (F32)args_gyro        : ジャイロセンサ値
//   (F32)args_gyro_offset : ジャイロセンサオフセット値
//   (F32)args_theta_m_l   : 左モータエンコーダ値
//   (F32)args_theta_m_r   : 右モータエンコーダ値
//   (F32)args_battery     : バッテリ電圧値(mV)
// 戻り値  :
//   (S8*)ret_pwm_l        : 左モータPWM出力値
//   (S8*)ret_pwm_r        : 右モータPWM出力値
...
//          (F32)100,                                  /* 前進最高速命令 */
//          (F32)0,                                    /* 無旋回命令 */
//          (F32)ecrobot_get_gyro_sensor(NXT_PORT_S4), /* ジャイロセンサ値 */
//          (F32)605,                                  /* 車体停止時のジャイロセンサ値 */
//          (F32)nxt_motor_get_count(NXT_PORT_C),      /* 左モータエンコーダ値 */
//          (F32)nxt_motor_get_count(NXT_PORT_B),      /* 右モータエンコーダ値 */
//          (F32)ecrobot_get_battery_voltage(),        /* バッテリ電圧値(mV) */
//          &pwm_l,                                    /* 左モータPWM出力値 */
//          &pwm_r);                                   /* 右モータPWM出力値 */
//*****************************************************************************
//*****************************************************************************
